...
export default function SkelbiaiPage() {
  ...
  const fetchSkelbimai = async () => {
    try {
      ...
      if (filtras.lotyniskas_pav) {
        params.append("lotyniskas_pav", filtras.lotyniskas_pav);
      }
      if (filtras.amzius) {
      ...
      data = data.filter((s: Skelbimas) => {
        if (filtras.pavadinimas && !s.pavadinimas.toLowerCase().includes(filtras.pavadinimas.toLowerCase())) return false;
        if (filtras.lotyniskas_pav && !s.lotyniskas_pav?.toLowerCase().includes(filtras.lotyniskas_pav.toLowerCase())) return false;
        if (filtras.amzius && s.amzius !== parseInt(filtras.amzius)) return false;
        if (filtras.aukstis && s.aukstis !== parseInt(filtras.aukstis)) return false;
        if (filtras.vieta && !s.vieta?.toLowerCase().includes(filtras.vieta.toLowerCase())) return false;
        if (filtras.kilme && !s.kilme?.toLowerCase().includes(filtras.kilme.toLowerCase())) return false;
        return true;
      });
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFiltras({ ...filtras, rusisMins: parseFloat(e.target.value) })
                  }
                  className="w-full"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFiltras({ ...filtras, rusisMaks: parseFloat(e.target.value) })
                  }
                  className="w-full"
                />
              </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFiltras({ ...filtras, pavadinimas: e.target.value })}
                  placeholder="pvz. Eglė"
              ...
              <div>
                ...
                <input
                  type="text"
                  value={filtras.lotyniskas_pav}
                  onChange={(e) => setFiltras({ ...filtras, lotyniskas_pav: e.target.value })}
                  placeholder="pvz. Picea"
                  className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-sm"
                />
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFiltras({ ...filtras, amzius: e.target.value })}
                  placeholder="2"
                  min="0"
                  className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-sm"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFiltras({ ...filtras, aukstis: e.target.value })}
                  placeholder="40"
                  min="0"
                  className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-sm"
                />
              ...
              <div>
                ...
                <input
                  type="text"
                  value={filtras.vieta}
                  onChange={(e) => setFiltras({ ...filtras, vieta: e.target.value })}
                  placeholder="Vilnius"
                  className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-sm"
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={filtras.kilme}
                  onChange={(e) => setFiltras({ ...filtras, kilme: e.target.value })}
                  placeholder="Lietuva"
                  className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-sm"
                />
              </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-12 text-center">
            <p className="text-zinc-600 dark:text-zinc-400">
              Nėra skelbiimų, atitinkančių jūsų kriterijus
            </p>
          </div>
        ) : (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {skelbimai.map((skelbimas) => (
              ...
              >
                <div className="mb-4 flex h-48 items-center justify-center rounded-xl bg-green-100 dark:bg-green-900/30 text-5xl">
                  🌲
                </div>

                <h3 className="font-semibold text-lg">{skelbimas.pavadinimas}</h3>
                ...
                <div className="mt-4 space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
                  ...
                  {skelbimas.aukstis && <p>📏 Aukštis: {skelbimas.aukstis} cm</p>}
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  >
                    {skelbimas.statusas === "galimas" ? "Yra" : "Ribotai"}
                  </span>
                </div>
                ...
                {skelbimas.imone_pavadinimas && (
                  <div className="mt-4 rounded-lg bg-zinc-100 dark:bg-zinc-800 p-3 text-xs">
                    <p className="font-semibold text-zinc-900 dark:text-zinc-100">{skelbimas.imone_pavadinimas}</p>
                    {skelbimas.imone_miestas && <p className="text-zinc-600 dark:text-zinc-400">📍 {skelbimas.imone_miestas}</p>}
                    {skelbimas.imone_tel_nr && <p className="text-zinc-600 dark:text-zinc-400">📞 {skelbimas.imone_tel_nr}</p>}
                  </div>
                )}

                <Link href={`/skelbimas/${skelbimas.id_Skelbimas}`} className="mt-4 w-full inline-flex justify-center rounded-lg border border-green-700 dark:border-green-400 bg-white dark:bg-transparent py-2 text-sm font-medium text-green-700 dark:text-green-400 transition-colors hover:bg-green-50 dark:hover:bg-green-900/20">
                  Sužinoti daugiau
                </Link>
              </div>
            ))}
